...
|                        | Hosted                           | Direct / BYOK        |
| ---------------------- | -------------------------------- | -------------------- |
| TypeScript entry point | `FileRouter`                     | `DirectFileRouter`   |
...
Hosted processing is the shortest path to production. FileRouter stores the document, creates a durable job, runs each selected provider independently, and retains each normalized result for retrieval.
...
Hosted binary uploads are limited to **100 MB**. Stored documents and completed results expire after **7 days**, and job records are retained for **30 days**.

Public URL inputs are fetched once into FileRouter storage before any provider
